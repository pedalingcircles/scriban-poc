...
public class Export
{
    ...
    static public string RecordsToCSV(SessionMessages session, HashSet<string> RecordFieldNames, HashSet<string> RecordDeveloperFieldNames)
    {
        ...
        stringBuilder.Append($"{string.Join(",", RecordFieldNames)},");

        if (RecordDeveloperFieldNames.Count > 0)
        {
            stringBuilder.Append($"developerdata_{string.Join(",developerdata_", RecordDeveloperFieldNames).Replace(" ", "_")},");
        }
        ...
        foreach (RecordMesg record in session.Records)
        {
            ...
            foreach (string fieldName in RecordFieldNames)
            {
